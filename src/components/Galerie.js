...
export default class Galerie extends React.Component {
  constructor(props) {
    ...
    this.state = {
      ...
      nextObject:
        DysgnathieGallery.media[mod(-1, DysgnathieGallery.media.length)],
    ...
    this.setState(
      ...
      () => {
        ...
        if (this.state.openedObject.type === "video") {
          let poster = this.state.videoContent[this.props.chosenSubjectIndex]
            .videos[parseInt(this.state.openedObject.videoId, 10)].poster;
          let source =
            "https://" +
            this.state.videoContent[this.props.chosenSubjectIndex].videos[
              parseInt(this.state.openedObject.videoId, 10)
            ].host +
            ":" +
            this.state.videoContent[this.props.chosenSubjectIndex].videos[
              parseInt(this.state.openedObject.videoId, 10)
            ...
            "/playlist.m3u8";
            let cuepoints = this.state.videoContent[this.props.chosenSubjectIndex].videos[
              parseInt(this.state.openedObject.videoId, 10)
            ].cuepoints;

          console.log(source);
          let cuepointsNameList = [];
          for (var i = 0; i < this.state.cuepoints.length; i++) {
            cuepointsNameList.push(<ListGroupItem tag="a" href="#" onClick="jumpToCuepointHandler" id={i} key={i}>{this.state.cuepoints[i].caption}</ListGroupItem>);
          }
          this.setState({
            sourceOfLoadedVideo: source,
            posterOfLoadedVideo: poster,
            cuepoints: cuepoints,
            cuepointsNameList: cuepointsNameList
          });
        }
      }
    );
  ...
  previousObjectHandler() {
    this.setState(
      {
        openedObjectIndex: mod(
          parseInt(this.state.openedObjectIndex, 10) - 1,
          this.state.listOfMedia.length, 10
        )
      },
      () => {
        this.setState({
          openedObject: this.state.listOfMedia[this.state.openedObjectIndex],
          nextObject: this.state.listOfMedia[
            mod(
              parseInt(this.state.openedObjectIndex, 10) + 1,
              this.state.listOfMedia.length, 10
            )
          ],
          previousObject: this.state.listOfMedia[
            mod(
              parseInt(this.state.openedObjectIndex, 10) - 1,
              this.state.listOfMedia.length, 10
            )
  ...
  chooseTabHandler(tab, e) {
    ...
    for (
      var i = 0;
      i < this.state.galleryContent[this.props.chosenSubjectIndex].media.length;
      i++
    ) {
      if (
        this.state.galleryContent[this.props.chosenSubjectIndex].media[i]
          .type === "image"
      ) {
        images.push(
          this.state.galleryContent[this.props.chosenSubjectIndex].media[i]
    ...
    switch (tab) {
      case "1":
        this.setState({
          listOfMedia: this.state.galleryContent[this.props.chosenSubjectIndex]
            .media
        });
        break;
      case "2":
  ...
  render() {
    console.log(this.state.cuepointsNameList);
    let videoJsOptions = {
      autoplay: false,
      controls: true,
      fluid: false,
          ...
          <div id="overlayContent">
            ...
            {this.state.openedObject.type === "video" && (
              <div id="videoContent">
                ...
                <MyVideoPlayer {...videoJsOptions} ref={(el) => this.player = el} />
                  <ListGroup flush>
                    {this.state.cuepointsNameList}
                  </ListGroup>
              </div>
            )}
            ...
            <div id="footer">
              <div className="left">
                <Button
                  color="primary"
                  className="mediaBtns"
                  onClick={this.previousObjectHandler}
                >
                  Zurück
                </Button>
              </div>
              <div id="position">
                {this.state.openedObjectIndex + 1}/{
                  this.state.listOfMedia.length
                }
              </div>
              <div className="right">
                <Button
                  color="primary"
                  className="mediaBtns"
                  onClick={this.nextObjectHandler}
                >
                  Weiter
                </Button>
              </div>
